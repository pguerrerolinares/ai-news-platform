...
class TestStoreClassifiedItems:
    """Tests for store_classified_items against real PostgreSQL."""
    ...
    async def test_stores_items_in_db(self, db_session):
        ...
        count = await store_classified_items(db_session, items)
    ...
    async def test_dedup_by_content_hash(self, db_session):
        ...
        await store_classified_items(db_session, [item])
        count2 = await store_classified_items(db_session, [item])
...
class TestSaveBriefing:
    """Tests for save_briefing against real PostgreSQL."""
