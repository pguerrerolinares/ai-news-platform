...
Key defaults: `OPENAI_BASE_URL=api.moonshot.cn/v1`, `OPENAI_MODEL=kimi-k2.6`, `OPENAI_TEMPERATURE=0.6`, `OPENAI_EXTRA_BODY={"thinking": {"type": "disabled"}}`, `MIN_RELEVANCE_SCORE=0.8` (see ADR-003; 4xx/quota LLM errors fail the run, only transient errors fall back to keywords; a non-parseable LLM response — empty, truncated, or not a JSON array — also falls back to keywords for that batch, logging `llm_response_not_parseable` and incrementing `ainews_llm_parse_failures_total`; per-item title/text/url are wrapped in `<item_content idx="N">` delimiters with `<`/`>` escaped, and the system prompt states that content is untrusted data, never instructions — see #15; `scripts/bench_classifier.py --dataset <labels.jsonl> [--dry-run]` reproduces the ADR-003 recall/FP/cost table against a hand-labeled dataset), `EMBEDDING_MODEL=text-embedding-3-small`, `ENABLED_SOURCES=hackernews,hackernews_leading,arxiv,rss,github,github_search,huggingface,webscraper` (Reddit disabled)
